...
tags = {
       names = { "term", "web", "files", "media", "tools", "dev" },       
...
mysystem = {
    ...
    { "Power Management" , terminal .. " -e sudo powertop " },
    { "Task manager" , tasks },
    { "Restart", awesome.restart }, 
    { "Suspend", terminal .. " -e sudo systemctl suspend " }, 
    { "Hibernate", terminal .. " -e sudo systemctl hibernate " }, 
...
local function create_calendar()
   ...
   local first_day_in_week =
      os.date("%w", first_day)
   local result = "dim lun mar mer jeu ven sam\n"
...
fshwidget = wibox.widget.textbox()
    ...
    function (widget, args)
        ...
        elseif args["{/home used_p}"] >= 99 and args["{/home used_p}"] <= 100 then
            naughty.notify({ title = "Warning", text = "/home partition full!\nCleanup required.",
                             timeout = 10,
                             position = "top_right",
...
vicious.register(batwidget, vicious.widgets.bat,
     function (widget, args)
          ...
          elseif (args[2] <= 5 and batstate() == 'Discharging') then
               naughty.notify({
                    text = "Going to shut-down...",
                    title = "Battery almost exhausted!",
                    position = "top_right",
                    timeout = 1,
                    fg="#000000",
                    bg="#ffffff",
...
globalkeys = awful.util.table.join(
    ...
    -- Dropdown terminal
    awful.key({ modkey,	          }, "w",     function () scratch.drop(terminal) end),
